...
def multithread_solve(parameters):
    ...
    entry = {'pid': index, 'target': target, 'guess': answer, 
             'steps': steps, 'step_lst': step_lst, 'time': round(time_interval, 2)}
    # ground truth
    data_json = json.load(open(args.data_path))
    if 'precise_value' in data_json:
        ...
        if answer is not None:
            ...
            try:
                ...
                if all([x is not None for x in value_list]):
                    closest_index = min(range(len(value_list)), key=lambda i: abs(value_list[i] - answer))
                    entry['choice'] = chr(closest_index+65)
            except Exception as e:
                if args.debug_mode:
                    print("\035[0;0;41mError:\0353[0m", repr(e), value_list, answer)
...
if __name__ == '__main__':
    ...
    lst = ['input_diagram'] # range(2401, 3002)
    ...
    if args.debug_mode:
        ...
        solve_list = sorted(solve_list, key=lambda x: x['pid'])
        ...
        label = lst[0]
        log_file = open("output/logs/log-{}.log".format(label), "w")
        ...
        with open("output/pred_results/logic_{}.json".format(label), 'w') as f:
