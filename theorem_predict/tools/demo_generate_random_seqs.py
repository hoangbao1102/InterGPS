...
def predict(pid, SEQ_TYPE,text_logic_tables,diagram_logic_tables):
    print(pid)
    split = 'test'
    ...
    # read problem data
    ANSWER_INPUT_PATH = "input\data.json"
    with open(ANSWER_INPUT_PATH, "r") as f:
        data = json.load(f)
    if 'problem_type_graph' not in data:
        graph_type = []
    else:
        graph_type = data['problem_type_graph']  # ["Line"]
    if 'problem_type_goal' not in data:
        goal_type = []
    else:
...
TEXT_INPUT_PATH = "output/text/text_logic_forms.json"  # human annotated
DIAGRAM_INPUT_PATH = "output/diagram/diagram_logic_forms.json"  # human annotated
...
def seq_gen_demo():
    ...
    for SEQ_TYPE in ['random', 'fixed']:
        results = {}
        lst = ['input_diagram',SEQ_TYPE]
        # with Pool(20) as p:
        #     pred_list = p.map(predict, lst)  # multiple processing pools for 'predict' over 'lst'
        pred_list = predict('input_diagram', SEQ_TYPE, text_logic_tables, diagram_logic_tables)
        print(pred_list)
        # for data in pred_list:
        #     pid = data['id']
        #     results[pid] = data
        pid = pred_list['id']
        results[pid] = pred_list

        # Save result.json file
        JSON_FILE = "output/demo/pred_seqs_test_l{}_{}.json".format(MAX_LEN, SEQ_TYPE)
        print("Saving generated theorem sequences to {}".format(JSON_FILE))
        with open(JSON_FILE, 'w') as f:
            json.dump(results, f, indent=2, separators=(',', ': '))
